...
:-
    empty_nb_hashtbl(FHash, 63),
    nb_linkval(feature_hash,FHash).

cached_embed_list2(X,Range,Offset,F):-
    nb_getval(feature_hash,FHash),
    cached_embed_list(X,FHash,Range,Offset,F),
    nb_linkval(feature_hash,FHash).
cached_embed2(X,Range,Offset,F):-
    nb_getval(feature_hash,FHash),
