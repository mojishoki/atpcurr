...
def get_state_embedding(goal, path, lem, moregoals, S_DIM, embed_separately=False, pairs=False):
    if embed_separately:
        goal = get_onehot_list(goal, S_DIM, pairs)
        path = get_onehot_list(path, S_DIM, pairs)
        lem = get_onehot_list(lem, S_DIM, pairs)
        moregoals = get_onehot_list(moregoals, S_DIM,pairs)
        state = np.array((goal, path, lem, moregoals))
    else:
        assert (S_DIM % 4) == 0, "S_DIM should be divisible by 4"
        goal = get_onehot_list(goal, S_DIM//4, pairs)
        path = get_onehot_list(path, S_DIM//4, pairs)
        lem = get_onehot_list(lem, S_DIM//4, pairs)
        moregoals = get_onehot_list(moregoals, S_DIM//4, pairs)
        state = np.concatenate((goal, path, lem, moregoals))
    return state
...
class Indexer(object):
    ...
    def store_state(self, goal, path, lem, moregoals, pairs=False):
        goal = np.array(goal)
        path = np.array(path)
        lem = np.array(lem)
        moregoals = np.array(moregoals)
        ...
        if self.feature_list is None:
            state = get_state_embedding(goal, path, lem, moregoals, self.S_DIM, self.embed_separately, pairs)
        else:
    ...
    for file in problemdict:
        ...
        if dict["proof"] is not None:
            ...
            if dict["episodes"] == 0:
                ...
                success_ratio = dict["success"] * 1.0 / dict["episodes"]
